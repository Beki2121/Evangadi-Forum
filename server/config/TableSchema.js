  ...
  try {
    ...
    await dbConnection.query(`
      CREATE TABLE IF NOT EXISTS chat_history (
        id INT AUTO_INCREMENT PRIMARY KEY,
        session_id VARCHAR(255) NOT NULL,
        user_id INT NULL, -- NULL if anonymous chat
        role ENUM('user', 'model') NOT NULL,
        content TEXT NOT NULL,
        timestamp DATETIME DEFAULT CURRENT_TIMESTAMP,
        FOREIGN KEY (user_id) REFERENCES users(userid) ON DELETE SET NULL
    ...
    // Note: This relies on the 'answers' table already existing.
    // Ensure 'initializeDatabase' runs 'answers' table creation before 'questions' table creation or this alter.
    await addColumnIfNotExists(
      "questions",
      "solution_answer_id",
      "INT NULL",
